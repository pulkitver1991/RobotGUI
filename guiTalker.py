        ...
        while (1):
            try:
                if (time.time() - self.startTime) > self.file_read_timeout:
                    self.startTime = time.time()
                    behavior = self.check_behavior(path=self.homeDirectory + self.filepath)
                
                # For controlling the Piston of a particular robot
                if behavior == "piston":
                    with open (self.homeDirectory + self.filepath + '/piston.json', 'r') as f:
                        data = json.load(f)
                    ...
                    self.erase_behavior(path=self.homeDirectory + self.filepath)


                # For controlling the Speed of a particular robot
                if behavior == "cmd_vel":
                    with open (self.homeDirectory + self.filepath + '/cmd_vel.json', 'r') as f:
                        data = json.load(f)
                    rid = data['id']
                    vel = float(data['vel'])
